		...
		try {
			for (int i = 0; i <= count; i += 1) { // 1초 간격
				synchronized (printWriter) {
					printWriter.println(date + getSecToTime(i) + "," + carDriving.getCarNum() + ","
							+ carDriving.getAccStep() + "," + carDriving.getBrkStep() + "," + carDriving.getWheelStep()
							+ "," + carDriving.getDirLightStep() + "," + carDriving.getSpeed() + ","
